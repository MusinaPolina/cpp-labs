...
void Employee::base_outputf(std::ostream& os) const {
    os << BinManipulator::write_c_str(_name.c_str()) << BinManipulator::write_le_int32(_base_salary);
}

void Employee::base_inputf(std::istream& is) {
    char *name = new char[NAME_SIZE];
    is >> BinManipulator::read_c_str(name, NAME_SIZE) >> BinManipulator::read_le_int32(_base_salary);
    _name = name;
    delete[] name;
}
...
std::ofstream& operator<<(std::ofstream &os, const Developer &dev) {
    os << BinManipulator::write_le_int32(1);
    dev.base_outputf(os);
    os << BinManipulator::write_bool(dev._has_bonus);
    return os;
}

std::ifstream& operator>>(std::ifstream &is, Developer &dev) {
    dev.base_inputf(is);
    is >> BinManipulator::read_bool(dev._has_bonus);
    return is;
}
...
std::ofstream& operator<<(std::ofstream &os, const SalesManager &sMan) {
    os << BinManipulator::write_le_int32(2);
    sMan.base_outputf(os);
    os << BinManipulator::write_le_int32(sMan._sold_nm) << BinManipulator::write_le_int32(sMan._price);
    return os ;
}

std::ifstream& operator>>(std::ifstream &is, SalesManager &sMan) {
    sMan.base_inputf(is);
    is >> BinManipulator::read_le_int32(sMan._sold_nm) >> BinManipulator::read_le_int32(sMan._price);
    return is;
}
...
std::ofstream& operator<<(std::ofstream &os, const EmployeesArray &array) {
    os << BinManipulator::write_le_int32(array._employees.size());
...
std::ifstream& operator>>(std::ifstream &is, EmployeesArray &array) {
    int32_t sz;
    is >> BinManipulator::read_le_int32(sz);
    for (int32_t i = 0; i < sz; i++) {
        int32_t type;
        is >> BinManipulator::read_le_int32(type);
        if (type == 1) {
